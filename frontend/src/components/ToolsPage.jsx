...
export default function ToolsPage({
  ...
  return (
    <div className="tools-page">
      ...
      {/* INTERACTIVE TOOL RUNNER MODAL / DRAWER */}
      {activeTool && (
        <div className="tool-runner-overlay" onClick={closeRunner}>
          <div className="tool-runner-modal" onClick={(e) => e.stopPropagation()}>
            {/* Modal Header */}
            <div className="tool-runner-header">
              <div className="tool-runner-title-area">
                <div
                  className="tool-runner-icon-badge"
                  style={{
                    color: getCategoryTheme(activeTool.cat).color,
                    background: getCategoryTheme(activeTool.cat).bg,
                  }}
                >
                  <activeTool.icon size={20} />
                </div>
                <div className="tool-runner-title-info">
                  <h3 className="tool-runner-title">{activeTool.name}</h3>
                  <p className="tool-runner-subtitle">
                    Supports {activeTool.hint} (max 40MB)
                  </p>
                </div>
              </div>
              <button
                className="tool-runner-close-btn"
                onClick={closeRunner}
                title="Close"
            ...
            <div className="tool-runner-body">
              ...
              {!result && (
                <>
                  ...
                  >
                    ...
                    {isRunning ? (
                      <div className="dropzone-converting">
                        <Loader2 size={36} className="spin-icon" style={{ color: getCategoryTheme(activeTool.cat).color }} />
                        <h4>Processing Document…</h4>
                        <p>Please wait while your file is safely processed</p>
                        <div className="conversion-progress-bar">
                          <div
                            className="progress-fill"
                            style={{
                              background: `linear-gradient(90deg, ${getCategoryTheme(activeTool.cat).color}, #ec4899)`,
                            }}
                          />
                        </div>
                      </div>
                      ...
                      <div className="dropzone-idle">
                        <div className="dropzone-icon" style={{ color: '#22c55e' }}>
                          <CheckCircle size={32} />
                        </div>
                        <h4
                        ...
                        <p className="dropzone-sub">
                          {(files.reduce((acc, f) => acc + f.size, 0) / (1024 * 1024)).toFixed(2)} MB · Click to choose different file{activeTool.multi ? 's' : ''}
                        </p>
                      </div>
                    ) : (
                      <div className="dropzone-idle">
                        <div
                          ...
                          style={{ color: getCategoryTheme(activeTool.cat).color }}
                        >
                          <UploadCloud size={34} />
                        </div>
                        <h4>Choose file{activeTool.multi ? 's' : ''} or drag & drop here</h4>
                        <p className="dropzone-sub">Accepts {activeTool.hint} (up to 40 MB)</p>
                        <button type="button" className="btn-browse-file">Browse Files</button>
                      </div>
                    )}
                  </div>

                  {/* Extra Parameters */}
                  {activeTool.param && files.length > 0 && !isRunning && (
                    <div style={{ marginTop: '14px' }}>
                      {activeTool.param === 'angle' ? (
                        <div>
                        ...
                        <div>
                          <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '6px' }}>
                            {paramLabel[activeTool.param] || 'Parameter'}
                          </label>
                          <input
                            type={activeTool.param === 'password' ? 'password' : 'text'}
                            value={paramText}
                            onChange={(e) => setParamText(e.target.value)}
                            placeholder={paramLabel[activeTool.param]}
                            style={{
                              width: '100%',
                              padding: '11px 12px',
                              borderRadius: '10px',
                  ...
                  {/* Error Notification */}
                  {errorMsg && (
                    <div className="converter-error-box" style={{ marginTop: '12px' }}>
                      <AlertCircle size={16} />
                      <span>{errorMsg}</span>
                    </div>
                  )}
                  ...
                  {files.length > 0 && !isRunning && (
                    <button
                      ...
                      style={{
                        ...
                        background: getCategoryTheme(activeTool.cat).color,
                        color: '#fff',
                        border: 'none',
                        padding: '12px 18px',
                        borderRadius: '11px',
                ...
                <div className="converter-success-box">
                  <div className="success-header">
                    ...
                    <div>
                      ...
                      <p className="success-sub">{activeTool.name} completed successfully</p>
                    </div>
                  </div>
                  <div
                    style={{
                  ...
                  <div className="convert-another-wrapper" style={{ display: 'flex', gap: '10px', marginTop: '14px' }}>
                    <button
                    ...
                    <button className="btn-convert-another" onClick={resetRunner}>
                      Process Another
                    </button>
                    <button className="btn-convert-another" onClick={closeRunner}>
                      Back to Tools
                    </button>
                  </div>
                </div>
              ...
              {/* Result View: Downloadable File */}
              {result && !result.text && (
                <div className="converter-success-box">
                  <div className="success-header">
                    <CheckCircle size={24} className="success-icon" />
                    <div>
                      <h4 className="success-title">Your file is ready!</h4>
                  ...
                  <div className="success-actions">
                    <button
                      type="button"
                      onClick={() => downloadFile(result.download_url, result.output_name)}
                  ...
                  <div className="convert-another-wrapper" style={{ display: 'flex', gap: '10px' }}>
                    <button className="btn-convert-another" onClick={resetRunner}>
