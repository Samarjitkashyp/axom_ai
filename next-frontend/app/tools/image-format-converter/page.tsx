...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = cms?.meta_title || 'Free Image Format Converter Online — JFIF, WebP, SVG, HEIC to PNG/JPG | Axom AI';
  const description =
    cms?.meta_description ||
    'Convert any image format — JFIF to PNG, WebP to PNG, SVG to PNG, HEIC to JPG, AVIF to WebP, and more. Supports all modern and legacy formats. 100% free, no watermarks.';
  ...
  const keywords = cms?.meta_keywords
    ...
    : [
        'image format converter', 'jfif to png', 'webp to png', 'svg to png',
        'heic to jpg', 'avif to webp', 'bmp to png', 'tiff to jpg',
        'convert image format online free', 'gif to png', 'ico to png',
        'axom ai tools', 'best free image converter',
      ];
...
export default async function ImageFormatConverterPage() {
  ...
  const webAppSchema = {
    ...
    description: cms?.meta_description || 'Free online utility by Axom AI to convert between any image format — JFIF, WebP, SVG, BMP, TIFF, GIF, ICO, AVIF, HEIC, PNG, JPG.',
    applicationCategory: 'BusinessApplication',
    operatingSystem: 'All (Web Browser, Windows, macOS, Linux, Android, iOS)',
    offers: { '@type': 'Offer', price: '0', priceCurrency: 'USD' },
    featureList: [
      '100% Free image format conversion', 'Supports JFIF, WebP, SVG, BMP, TIFF, GIF, ICO, AVIF, HEIC, PNG, JPG',
      'Output to PNG, JPG, or WebP', 'No watermark added to output files',
      'Zero user registration or login required', '256-bit SSL encrypted transfer with automatic file purging',
    ],
  ...
  const howToSchema = {
    ...
    name: cms?.how_it_works_title || 'How to Convert Image Formats in 3 Easy Steps',
    description: cms?.how_it_works_subheading || 'Step-by-step guide to convert between image formats online in seconds.',
    totalTime: 'PT30S',
    step: [
      { '@type': 'HowToStep', position: 1, name: cms?.step_1_title || 'Upload Your Image', text: cms?.step_1_desc || 'Drag and drop your image (JFIF, WebP, SVG, BMP, TIFF, GIF, ICO, AVIF, HEIC, PNG, JPG) into the converter.', url: 'https://aiaxom.co.in/tools/image-format-converter/#converter' },
      { '@type': 'HowToStep', position: 2, name: cms?.step_2_title || 'Choose Target Format', text: cms?.step_2_desc || 'Select your desired output format — PNG (lossless), JPG (compressed), or WebP (modern, best quality/size).', url: 'https://aiaxom.co.in/tools/image-format-converter/#converter' },
      { '@type': 'HowToStep', position: 3, name: cms?.step_3_title || 'Download Converted Image', text: cms?.step_3_desc || 'Download your converted image directly to your device. Clean output with no quality loss.', url: 'https://aiaxom.co.in/tools/image-format-converter/#converter' },
    ],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Image Format Converter', item: cms?.canonical_url || 'https://aiaxom.co.in/tools/image-format-converter/' },
    ],
  };
  ...
  const comparisonRows = cms?.comparison_matrix && cms.comparison_matrix.length > 0 ? cms.comparison_matrix : [
    ...
    { feature: 'Format Support', axom: 'JFIF, WebP, SVG, BMP, TIFF, GIF, ICO, AVIF, HEIC', other: 'PNG, JPG only', paid: 'Most formats', axom_check: true, other_check: false },
    { feature: 'Watermarks', axom: 'Never (100% Clean)', other: 'Added to output', paid: 'Clean (Paid only)', axom_check: true, other_check: false },
    ...
    { feature: 'Output Options', axom: 'PNG, JPG, or WebP — user choice', other: 'Fixed output format', paid: 'Multiple formats', axom_check: true, other_check: false },
  ];
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <section className="text-center max-w-3xl mx-auto mb-10 sm:mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-fuchsia-500/10 via-purple-500/10 to-indigo-500/10 border border-purple-500/20 text-purple-300 text-xs font-semibold mb-4 shadow-sm">
              ...
              <span>{cms?.hero_badge_text || '⚡ Free: 20 Files / Day • Supports All Modern Image Formats'}</span>
            </div>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight leading-tight sm:leading-tight mb-4">
              ...
              <span className="gradient-text">{cms?.hero_heading_highlight || 'Image Format'}</span>{' '}
            ...
            <p className="text-sm sm:text-base md:text-lg text-slate-300 leading-relaxed max-w-2xl mx-auto">
              {cms?.hero_description || 'Convert any image format — JFIF to PNG, WebP to PNG, SVG to PNG, HEIC to JPG, AVIF to WebP, and more. Supports all next-gen and legacy formats. Free accounts can convert up to 20 files per day!'}
            </p>
          </section>

          <section id="converter" className="mb-20">
            <ImageFormatConverter freeDailyLimit={cms?.free_daily_limit} proBatchLimit={cms?.pro_batch_limit} maxFileSizeMb={cms?.max_file_size_mb} />
          </section>

          <section className="mb-20 max-w-4xl mx-auto">
            <div className="text-center mb-10">
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">{cms?.how_it_works_title || 'How to Convert Image Formats in 3 Easy Steps'}</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="p-6 rounded-2xl bg-slate-900/50 border border-white/5 hover:border-purple-500/30 transition text-center relative group">
                ...
                <h3 className="text-base font-bold text-white mb-2">{cms?.step_1_title || 'Upload Image'}</h3>
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{cms?.step_1_desc || 'Drag and drop your image file (JFIF, WebP, SVG, BMP, TIFF, GIF, ICO, AVIF, HEIC, PNG, or JPG) into the converter.'}</p>
              </div>
              <div className="p-6 rounded-2xl bg-slate-900/50 border border-white/5 hover:border-purple-500/30 transition text-center relative group">
                ...
                <h3 className="text-base font-bold text-white mb-2">{cms?.step_2_title || 'Choose Target Format'}</h3>
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{cms?.step_2_desc || 'Select your desired output format from the dropdown — PNG (lossless), JPG (compressed), or WebP (modern, best quality/size).'}</p>
              </div>
              <div className="p-6 rounded-2xl bg-slate-900/50 border border-white/5 hover:border-purple-500/30 transition text-center relative group">
                ...
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{cms?.step_3_desc || 'Download your converted image directly to your device. No watermarks, no quality loss.'}</p>
              </div>
            </div>
          </section>
          ...
          <section className="mb-20">
            <div className="text-center mb-12">
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-3">{cms?.why_title || 'Why Axom AI Image Format Converter is the Superior Choice'}</h2>
              <p className="text-slate-400 text-sm max-w-xl mx-auto">{cms?.why_subheading || 'Engineered for designers, developers, and anyone who works with images across different platforms.'}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="p-6 rounded-2xl bg-slate-900/40 border border-white/5 hover:border-purple-500/30 transition">
                ...
                <h3 className="text-base font-bold text-white mb-2">{cms?.benefit_1_title || 'Universal Format Support'}</h3>
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{cms?.benefit_1_desc || 'Handles JFIF, WebP, SVG, BMP, TIFF, GIF, ICO, AVIF, HEIC, PNG, and JPG — every modern and legacy format in one tool.'}</p>
              </div>
              <div className="p-6 rounded-2xl bg-slate-900/40 border border-white/5 hover:border-purple-500/30 transition">
                ...
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{cms?.benefit_2_desc || 'No hidden subscription traps or promotional watermarks. Clean converted images ready for any use.'}</p>
              </div>
              <div className="p-6 rounded-2xl bg-slate-900/40 border border-white/5 hover:border-purple-500/30 transition">
              ...
              <div className="p-6 rounded-2xl bg-slate-900/40 border border-white/5 hover:border-purple-500/30 transition">
                ...
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{cms?.benefit_5_desc || 'High-speed optimized micro-services convert images in less than 3 seconds.'}</p>
              </div>
              <div className="p-6 rounded-2xl bg-slate-900/40 border border-white/5 hover:border-purple-500/30 transition">
                <div className="w-10 h-10 rounded-xl bg-cyan-500/15 text-cyan-400 flex items-center justify-center mb-4"><Layers size={20} /></div>
                <h3 className="text-base font-bold text-white mb-2">{cms?.benefit_6_title || 'Flexible Output Options'}</h3>
                <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">{cms?.benefit_6_desc || 'Choose between PNG (lossless with transparency), JPG (small compressed files), or WebP (modern best quality/size ratio).'}</p>
              </div>
            </div>
          </section>
          ...
          <section className="mb-20 max-w-4xl mx-auto">
            <div className="text-center mb-8">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2">{cms?.comparison_title || 'Axom AI vs. Traditional Image Format Converters'}</h2>
          ...
          <section className="mb-20 max-w-4xl mx-auto p-6 sm:p-8 rounded-3xl bg-gradient-to-b from-slate-900/60 to-slate-950/80 border border-white/10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 text-xs sm:text-sm">
              <div className="space-y-1"><div className="text-slate-400 text-xs uppercase tracking-wider font-semibold">Supported Inputs</div><div className="text-white font-medium">{cms?.tech_spec_inputs || 'JFIF, WebP, SVG, BMP, TIFF, GIF, ICO, AVIF, HEIC, PNG, JPG'}</div></div>
              <div className="space-y-1"><div className="text-slate-400 text-xs uppercase tracking-wider font-semibold">Output Formats</div><div className="text-white font-medium">{cms?.tech_spec_output || 'PNG / JPG / WebP'}</div></div>
          ...
          <section className="mb-20">
            <div className="text-center mb-10">
              ...
              <p className="text-slate-400 text-sm max-w-lg mx-auto">{cms?.faq_section_subheading || 'Got questions about image format conversion? Find verified answers below.'}</p>
            </div>
          ...
          <section className="text-center p-8 sm:p-12 rounded-3xl bg-gradient-to-r from-fuchsia-900/30 via-purple-900/20 to-indigo-900/30 border border-purple-500/30 shadow-2xl">
            <h2 className="text-2xl sm:text-3xl font-extrabold text-white mb-3">{cms?.cta_title || 'Convert Any Image Format in Seconds'}</h2>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href={cms?.cta_btn_primary_url || '#converter'} className="w-full sm:w-auto px-7 py-3.5 rounded-xl bg-gradient-to-r from-fuchsia-600 to-purple-600 text-white font-bold text-sm shadow-lg shadow-fuchsia-600/30 hover:scale-105 transition">{cms?.cta_btn_primary_text || 'Upload Image Now'}</a>
