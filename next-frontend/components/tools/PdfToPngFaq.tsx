...
import { PDF_TO_PNG_FAQS, FaqItem } from './pdfToPngData';

interface PdfToPngFaqProps {
  faqs?: FaqItem[];
}

export default function PdfToPngFaq({ faqs }: PdfToPngFaqProps) {
  const [openIndex, setOpenIndex] = useState<number | null>(0);
  const items = faqs && faqs.length > 0 ? faqs : PDF_TO_PNG_FAQS;
