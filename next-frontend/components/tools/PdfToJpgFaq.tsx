...
import { PDF_TO_JPG_FAQS, FaqItem } from './pdfToJpgData';

interface PdfToJpgFaqProps {
  faqs?: FaqItem[];
}

export default function PdfToJpgFaq({ faqs }: PdfToJpgFaqProps) {
  const [openIndex, setOpenIndex] = useState<number | null>(0);
  const items = faqs && faqs.length > 0 ? faqs : PDF_TO_JPG_FAQS;
