...
function getTodayUsageKey(): string {
  ...
  return `axom_p2j_usage_${today}`;
}

export interface PdfToJpgConverterProps {
...
export default function PdfToJpgConverter({
    ...
    async (fileList: FileList | File[]) => {
      ...
      for (const f of rawArr) {
        ...
        if (!ALLOWED_EXTS.includes(ext)) {
          setErrorMsg(`"${f.name}" is not a supported file. Only PDF files (${ALLOWED_EXTS.join(', ')}) are accepted.`);
          return;
        }
  ...
  const handleConvert = async () => {
    ...
    const timer1 = setTimeout(() => {
      setProgressMsg(isBatch ? `Extracting pages as high-resolution JPG for ${files.length} files...` : 'Extracting pages as high-resolution JPG images...');
    }, 900);
    const timer2 = setTimeout(() => {
      setProgressMsg(isBatch ? 'Bundling converted images into package...' : 'Rendering high-definition JPG output...');
    }, 2000);
    ...
    try {
      ...
      setResult({
        ...
        outputName: data.output_name || (isBatch ? `converted_${files.length}_pdfs.zip` : `${files[0].name.replace(/\.[^/.]+$/, '')}.jpg`),
        fileSize: data.file_size || (isBatch ? 'ZIP Archive' : 'JPG Image'),
        isBatch,
        count: files.length,
      });
    } catch (err: any) {
        ...
        <div className="relative rounded-[22px] bg-[#0c0d16]/95 backdrop-blur-xl p-5 sm:p-8 md:p-10 border border-white/5">
          ...
          <div className="text-center mb-6">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight">
              Convert PDF to High-Resolution JPG Images
            </h2>
          ...
          {result ? (
            <div className="py-8 px-4 text-center flex flex-col items-center animate-in zoom-in-95 duration-200">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-1">
                {result.isBatch ? `${result.count} PDFs Converted to JPG!` : 'Your JPG Images are Ready!'}
              </h3>
              <p className="text-sm text-slate-400 mb-6">
                {result.isBatch
                  ? `All ${result.count} PDF files were converted to high-resolution JPG images and packaged into a ZIP archive.`
                  : 'Each page has been extracted as a high-resolution JPG image. Multi-page PDFs are delivered as a ZIP file.'}
              </p>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full max-w-md">
                ...
                >
                  <Download size={18} />
                  <span>{result.isBatch ? 'Download All (ZIP)' : 'Download JPG Images'}</span>
                </a>
                ...
                >
                  {isConverting ? (
                    ...
                    <>
                      ...
                      <span>
                        {files.length > 1
                          ? `Convert All ${files.length} PDFs to JPG`
                          : 'Convert to JPG Now'}
                      </span>
                    </>
                  )}
            ...
            >
              ...
              <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto mb-4">
                Supports <span className="text-purple-300 font-semibold">.PDF</span> files up to {maxFileSizeMb || 25} MB. Each page is extracted as a high-resolution JPG image.
              </p>
