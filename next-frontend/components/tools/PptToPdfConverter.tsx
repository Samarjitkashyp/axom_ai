...
function getTodayUsageKey(): string {
  ...
  return `axom_ppt2pdf_usage_${today}`;
}

export interface PptToPdfConverterProps {
...
export default function PptToPdfConverter({
  ...
  proBatchLimit = 20,
  maxFileSizeMb = 50,
}: PptToPdfConverterProps = {}) {
  const effectiveMaxFileSize = (maxFileSizeMb || 50) * 1024 * 1024;
    ...
    async (fileList: FileList | File[]) => {
      ...
      for (const f of rawArr) {
        ...
        if (!ALLOWED_EXTS.includes(ext)) {
          setErrorMsg(`"${f.name}" is not a supported PowerPoint file. Please upload .ppt or .pptx files.`);
          return;
        }
        if (f.size > effectiveMaxFileSize) {
          setErrorMsg(`"${f.name}" exceeds the ${maxFileSizeMb || 50} MB limit.`);
          return;
        }
        validFiles.push(f);
      }
  ...
  const handleConvert = async () => {
    ...
    setProgressMsg(isBatch ? `Uploading ${files.length} PowerPoint files securely...` : 'Uploading PowerPoint securely...');

    const timer1 = setTimeout(() => {
      setProgressMsg(isBatch ? `Rendering ${files.length} presentations to PDF...` : 'Rendering slides to PDF...');
    }, 1200);
    const timer2 = setTimeout(() => {
      setProgressMsg(isBatch ? 'Building high-quality PDF documents...' : 'Building high-quality PDF document...');
    }, 3000);
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Conversion failed. Please verify your PowerPoint file and try again.');
      }
      ...
      setResult({
        ...
        outputName: data.output_name || (isBatch ? `converted_${files.length}_presentations.zip` : `${files[0].name.replace(/\.[^/.]+$/, '')}.pdf`),
        fileSize: data.file_size || (isBatch ? 'ZIP Archive' : 'PDF Document'),
        isBatch,
        count: files.length,
      });
    } catch (err: any) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      <div className="relative rounded-3xl p-1 bg-gradient-to-b from-orange-500/30 via-red-600/20 to-slate-900/40 shadow-2xl shadow-red-950/40">
        <div className="relative rounded-[22px] bg-[#0c0d16]/95 backdrop-blur-xl p-5 sm:p-8 md:p-10 border border-white/5">
          ...
          <div className="text-center mb-6">
            <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
              {isPro ? (
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gradient-to-r from-amber-500/20 to-red-500/20 border border-amber-500/40 text-amber-300 text-xs font-semibold shadow-sm">
                  <Crown size={13} className="text-amber-400" />
                  <span>Pro Plan Active: Unlimited Conversions & 20-File Batch Mode</span>
                </div>
              ) : (
                <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-orange-500/10 border border-orange-500/25 text-orange-300 text-xs font-semibold">
                  ...
                  <a
                    ...
                    className="text-red-400 hover:text-red-300 underline font-medium flex items-center gap-1 transition"
                  >
            ...
            <p className="text-xs sm:text-sm text-slate-400 mt-1 max-w-lg mx-auto">
              Free users get <span className="text-orange-300 font-semibold">20 files/day</span>. Premium subscribers can convert up to <span className="text-red-300 font-semibold">20 files at once</span> with Pro Batch Mode.
            </p>
            ...
            <div className="inline-flex items-center p-1 rounded-xl bg-slate-900 border border-white/10 mt-5 max-w-full">
              ...
              >
                ...
                <span>Single Presentation (Free)</span>
              </button>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition ${
                  mode === 'batch'
                    ? 'bg-gradient-to-r from-amber-500 to-red-600 text-white shadow-md shadow-red-600/30'
          ...
          {isDailyLimitReached && (
            <div className="mb-6 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs sm:text-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-in fade-in">
              ...
              <a
                href="https://chat.aiaxom.co.in/subscription"
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-red-600 text-white font-bold text-xs whitespace-nowrap shadow-md shadow-amber-500/20 hover:scale-105 transition"
              >
                Upgrade to Premium
              </a>
            </div>
          ...
          {result ? (
            <div className="py-8 px-4 text-center flex flex-col items-center animate-in zoom-in-95 duration-200">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-1">
                {result.isBatch ? `${result.count} PDF Documents Ready!` : 'Your PDF Document is Ready!'}
              </h3>
              <p className="text-sm text-slate-400 mb-6">
                {result.isBatch
                  ? `All ${result.count} presentations were converted to PDF and packaged into a ZIP archive.`
                  : 'Successfully converted your PowerPoint presentation to a high-quality PDF.'}
              </p>

              <div className="w-full max-w-md p-4 rounded-2xl bg-slate-900/80 border border-orange-500/20 flex items-center justify-between gap-3 mb-6">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full max-w-md">
                <a
                  ...
                  className="w-full sm:flex-1 py-3.5 px-6 rounded-xl bg-gradient-to-r from-orange-600 via-red-600 to-rose-600 text-white font-semibold text-sm shadow-xl shadow-red-600/25 hover:shadow-red-600/40 hover:scale-[1.02] active:scale-[0.98] transition flex items-center justify-center gap-2"
                >
                  <Download size={18} />
                  <span>{result.isBatch ? 'Download All (ZIP)' : 'Download PDF Document'}</span>
                </a>
            ...
            <div className="space-y-6 animate-in fade-in duration-150">
              {files.length === 1 ? (
                <div className="p-5 rounded-2xl bg-slate-900/90 border border-orange-500/25 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3.5 min-w-0">
                    <div className="w-12 h-12 rounded-xl bg-orange-500/15 border border-orange-500/30 flex items-center justify-center text-orange-400 shrink-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-400 mt-0.5 flex items-center gap-2">
                        ...
                        <span>&bull;</span>
                        <span className="uppercase text-orange-400 font-medium">PowerPoint</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-xs sm:text-sm text-slate-300 px-1">
                    <span className="font-semibold flex items-center gap-2">
                      ...
                      <span>{files.length} Presentations Selected (Batch Mode)</span>
                    </span>
                    ...
                    {files.map((f, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <Presentation size={16} className="text-orange-400 shrink-0" />
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-orange-600 via-red-600 to-rose-600 text-white font-bold text-base shadow-xl shadow-red-600/30 hover:shadow-red-600/50 hover:brightness-110 active:scale-[0.99] transition flex items-center justify-center gap-2.5 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {isConverting ? (
                    <>
                    ...
                    <>
                      ...
                      <span>
                        {files.length > 1
                          ? `Convert All ${files.length} Presentations to PDF`
                          : 'Convert to PDF Now'}
                      </span>
                    </>
                  )}
            ...
            <div
              ...
              className={`relative rounded-2xl border-2 border-dashed p-8 sm:p-12 text-center cursor-pointer transition duration-200 group ${
                dragActive
                  ...
                  : 'border-white/15 bg-slate-900/40 hover:border-orange-400/50 hover:bg-slate-900/70'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-tr from-orange-500/20 to-red-500/20 border border-orange-500/30 flex items-center justify-center text-orange-400 group-hover:scale-110 group-hover:text-red-300 transition duration-300">
                <UploadCloud size={32} />
              </div>

              <h3 className="text-lg sm:text-xl font-bold text-white mb-2 group-hover:text-orange-200 transition">
                {mode === 'batch' || isPro
                  ? 'Click to browse or drop up to 20 PowerPoint files'
                  : 'Click to browse or drop your PowerPoint file here'}
              </h3>

              <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto mb-4">
                Supports <span className="text-orange-300 font-semibold">.PPT</span> and <span className="text-orange-300 font-semibold">.PPTX</span> files up to {maxFileSizeMb || 50} MB
              </p>

              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-600/20 border border-orange-500/40 text-orange-300 text-xs font-semibold group-hover:bg-orange-600/30 transition">
                <Presentation size={14} />
                <span>
                  {mode === 'batch' || isPro
                    ? 'Select up to 20 Files (Batch Mode)'
                    : 'Select PowerPoint File (Free 20/day)'}
                </span>
              </div>
            </div>
          )}
      ...
      {showUpgradeModal && mounted && typeof document !== 'undefined' && createPortal(
        ...
        >
          ...
          >
            ...
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-amber-500/20 via-red-500/20 to-orange-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 mx-auto mb-4 shadow-lg shadow-amber-500/10">
              <Crown size={32} />
            </div>
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed mb-6">
              Free users can convert <strong className="text-orange-300">1 file at a time (up to 20 files per day)</strong>.
              <br className="hidden sm:inline" />
              Upgrade to <strong className="text-amber-300">Axom AI Premium</strong> to convert <strong className="text-white">up to 20 PowerPoint files simultaneously</strong> with 1-click batch ZIP download and unlimited daily access!
            </p>
            ...
            <div className="space-y-2.5 mb-6 text-left text-xs bg-slate-900/90 p-4 rounded-xl border border-white/5">
              ...
              <div className="flex items-center gap-2 text-slate-300">
                <CheckCircle2 size={14} className="text-emerald-400 shrink-0" />
                <span>Unlimited daily presentation conversions</span>
              </div>
              <div className="flex items-center gap-2 text-slate-300">
                <CheckCircle2 size={14} className="text-emerald-400 shrink-0" />
            ...
            <div className="flex flex-col gap-2.5">
              <a
                href="https://chat.aiaxom.co.in/subscription"
                className="w-full py-3.5 px-6 rounded-xl bg-gradient-to-r from-amber-500 via-red-600 to-orange-600 text-white font-bold text-sm shadow-xl shadow-red-600/30 hover:scale-[1.02] transition flex items-center justify-center gap-2"
              >
                <Crown size={16} />
