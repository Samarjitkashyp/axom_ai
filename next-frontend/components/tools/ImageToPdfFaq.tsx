...
import { IMAGE_TO_PDF_FAQS, FaqItem } from './imageToPdfData';

interface ImageToPdfFaqProps {
  faqs?: FaqItem[];
}

export default function ImageToPdfFaq({ faqs }: ImageToPdfFaqProps) {
  const [openIndex, setOpenIndex] = useState<number | null>(0);
  const items = faqs && faqs.length > 0 ? faqs : IMAGE_TO_PDF_FAQS;
