...
const ALLOWED_EXTS = ['.docx', '.doc', '.rtf', '.txt', '.odt', '.md'];
...
function getTodayUsageKey(): string {
  ...
  return `axom_w2p_usage_${today}`;
}

export interface WordToPdfConverterProps {
...
export default function WordToPdfConverter({
    ...
    async (fileList: FileList | File[]) => {
      ...
      for (const f of rawArr) {
        ...
        if (!ALLOWED_EXTS.includes(ext)) {
          setErrorMsg(`"${f.name}" is not a supported Word document (${ALLOWED_EXTS.join(', ')}).`);
          return;
        }
      ...
      if (!isPro && mode !== 'batch') {
        ...
        // Client preview for .docx
        if (singleFile.name.toLowerCase().endsWith('.docx')) {
          try {
            const buf = await singleFile.arrayBuffer();
            const { value } = await mammoth.convertToHtml({ arrayBuffer: buf });
            if (value && value.trim()) setDocPreviewHtml(value);
          } catch (err) {
            setDocPreviewHtml(null);
          }
        }
      ...
      // Single docx preview if only 1 file
      if (capped.length === 1 && capped[0].name.toLowerCase().endsWith('.docx')) {
        try {
          const buf = await capped[0].arrayBuffer();
          const { value } = await mammoth.convertToHtml({ arrayBuffer: buf });
          if (value && value.trim()) setDocPreviewHtml(value);
        } catch (err) {
          setDocPreviewHtml(null);
  ...
  const handleConvert = async () => {
    ...
    setProgressMsg(isBatch ? `Uploading ${files.length} documents securely...` : 'Uploading document securely...');

    const timer1 = setTimeout(() => {
      setProgressMsg(isBatch ? `Processing fonts, tables and margins for ${files.length} files...` : 'Preserving fonts, margins and tables...');
    }, 900);
    const timer2 = setTimeout(() => {
      setProgressMsg(isBatch ? 'Bundling converted PDFs into package...' : 'Rendering high-definition PDF...');
    }, 2000);
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Conversion failed. Please verify your document and try again.');
      }
      ...
      setResult({
        ...
        outputName: data.output_name || (isBatch ? `converted_${files.length}_documents.zip` : `${files[0].name.replace(/\.[^/.]+$/, '')}.pdf`),
          ...
          {result ? (
            <div className="py-8 px-4 text-center flex flex-col items-center animate-in zoom-in-95 duration-200">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-1">
                {result.isBatch ? `${result.count} PDFs Converted Successfully!` : 'Your PDF is Ready!'}
              </h3>
              <p className="text-sm text-slate-400 mb-6">
                {result.isBatch
                  ? `All ${result.count} Word documents were converted and packaged into a ZIP archive with layout preserved.`
                  : 'Successfully converted with original layout, fonts and tables preserved.'}
              </p>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full max-w-md">
                ...
                >
                  <Download size={18} />
                  <span>{result.isBatch ? 'Download All (ZIP)' : 'Download Converted PDF'}</span>
                </a>
            ...
            <div className="space-y-6 animate-in fade-in duration-150">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-400 mt-0.5 flex items-center gap-2">
                        ...
                        <span className="uppercase text-purple-400 font-medium">
                          {files[0].name.split('.').pop()} document
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    {docPreviewHtml && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 text-xs font-medium flex items-center gap-1.5 transition"
                      >
                        {showPreview ? <EyeOff size={14} /> : <Eye size={14} />}
                        <span>{showPreview ? 'Hide Preview' : 'Preview Content'}</span>
                      </button>
                    )}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-xs sm:text-sm text-slate-300 px-1">
                    <span className="font-semibold flex items-center gap-2">
                      ...
                      <span>{files.length} Documents Selected (Batch Mode)</span>
                    </span>
              ...
              {/* Optional Client Preview Section */}
              {showPreview && docPreviewHtml && (
                <div className="rounded-2xl p-4 bg-slate-950/80 border border-white/10 max-h-60 overflow-y-auto text-xs sm:text-sm text-slate-300 prose prose-invert max-w-none">
                ...
                >
                  {isConverting ? (
                    ...
                    <>
                      ...
                      <span>
                        {files.length > 1
                          ? `Convert All ${files.length} Documents to PDF`
                          : 'Convert to PDF Now'}
                      </span>
                    </>
                  )}
            ...
            >
              ...
              <h3 className="text-lg sm:text-xl font-bold text-white mb-2 group-hover:text-purple-200 transition">
                {mode === 'batch' || isPro
                  ? 'Click to browse or drop up to 20 Word documents'
                  : 'Click to browse or drop your Word document here'}
              </h3>
              
              <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto mb-4">
                Supports Microsoft Word <span className="text-purple-300 font-semibold">.DOCX</span>,{' '}
                <span className="text-purple-300 font-semibold">.DOC</span>, as well as .RTF, .TXT, and .ODT
              </p>
        ...
        >
          ...
          >
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed mb-6">
              ...
              Upgrade to <strong className="text-amber-300">Axom AI Premium</strong> to convert <strong className="text-white">up to 20 Word documents simultaneously</strong> with 1-click batch ZIP download and unlimited daily access!
            </p>
