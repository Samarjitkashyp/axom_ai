...
import { WORD_TO_PDF_FAQS, FaqItem } from './wordToPdfData';

interface WordToPdfFaqProps {
  faqs?: FaqItem[];
}

export default function WordToPdfFaq({ faqs }: WordToPdfFaqProps) {
  const [openIndex, setOpenIndex] = useState<number | null>(0);
  const items = faqs && faqs.length > 0 ? faqs : WORD_TO_PDF_FAQS;
