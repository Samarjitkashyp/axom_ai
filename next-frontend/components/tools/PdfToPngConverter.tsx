...
function getTodayUsageKey(): string {
  ...
  return `axom_p2p_usage_${today}`;
}

export interface PdfToPngConverterProps {
...
export default function PdfToPngConverter({
  ...
  const handleConvert = async () => {
    ...
    const timer1 = setTimeout(() => {
      setProgressMsg(isBatch ? `Rendering ${files.length} PDF files to high-resolution PNG...` : 'Rendering PDF pages to lossless PNG...');
    }, 900);
    const timer2 = setTimeout(() => {
      setProgressMsg(isBatch ? 'Bundling converted PNG images into package...' : 'Packaging PNG images for download...');
    }, 2000);
    ...
    try {
      ...
      setResult({
        ...
        outputName: data.output_name || (isBatch ? `converted_${files.length}_pdfs.zip` : `${files[0].name.replace(/\.[^/.]+$/, '')}.png`),
        fileSize: data.file_size || (isBatch ? 'ZIP Archive' : 'PNG Image'),
        isBatch,
        count: files.length,
      });
    } catch (err: any) {
          ...
          {result ? (
            <div className="py-8 px-4 text-center flex flex-col items-center animate-in zoom-in-95 duration-200">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-1">
                {result.isBatch ? `${result.count} PDFs Converted to PNG!` : 'Your PNG Images are Ready!'}
              </h3>
              <p className="text-sm text-slate-400 mb-6">
                {result.isBatch
                  ? `All ${result.count} PDF files were converted to lossless PNG images and packaged into a ZIP archive.`
                  : 'Successfully converted to lossless PNG images with transparency preserved. Multi-page PDFs are downloaded as a ZIP archive.'}
              </p>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full max-w-md">
                ...
                >
                  <Download size={18} />
                  <span>{result.isBatch ? 'Download All (ZIP)' : 'Download PNG Images'}</span>
                </a>
                ...
                >
                  {isConverting ? (
                    ...
                    <>
                      ...
                      <span>
                        {files.length > 1
                          ? `Convert All ${files.length} PDFs to PNG`
                          : 'Convert to PNG Now'}
                      </span>
                    </>
                  )}
            ...
            >
              ...
              <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto mb-4">
                ...
                Each page is converted to a lossless PNG image with transparency.
              </p>
