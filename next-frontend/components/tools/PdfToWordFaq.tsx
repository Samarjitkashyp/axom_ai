...
import { PDF_TO_WORD_FAQS, FaqItem } from './pdfToWordData';

interface PdfToWordFaqProps {
  faqs?: FaqItem[];
}

export default function PdfToWordFaq({ faqs }: PdfToWordFaqProps) {
  const [openIndex, setOpenIndex] = useState<number | null>(0);
  const items = faqs && faqs.length > 0 ? faqs : PDF_TO_WORD_FAQS;
