...
export default function ImageToPdfConverter({
  ...
  const processSelectedFiles = useCallback(
    async (fileList: FileList | File[]) => {
      ...
      if (rawArr.length > 1 && !isPro && mode === 'single') {
        // For images, we allow multi-file even in single mode since they merge into one PDF
        // But cap at batch limit
      }
      ...
      // For images, allow multi-file for free users too (they merge into one PDF)
      // But cap at effectiveBatchLimit for Pro, and a reasonable limit for free
      if (!isPro && validFiles.length > effectiveBatchLimit) {
      ...
      const capped = validFiles.slice(0, effectiveBatchLimit);
      setFiles((prev) => {
        const combined = [...prev, ...capped];
        return combined.slice(0, effectiveBatchLimit);
      });
    },
  ...
  const handleConvert = async () => {
    ...
    const timer1 = setTimeout(() => {
      setProgressMsg(isBatch ? `Processing ${files.length} images into PDF pages...` : 'Converting image to PDF...');
    }, 900);
    const timer2 = setTimeout(() => {
      setProgressMsg(isBatch ? 'Merging images into a single PDF document...' : 'Rendering high-quality PDF...');
    }, 2000);
    ...
    try {
      ...
      setResult({
        ...
        outputName: data.output_name || (isBatch ? `merged_${files.length}_images.pdf` : `${files[0].name.replace(/\.[^/.]+$/, '')}.pdf`),
        fileSize: data.file_size || 'PDF Document',
        isBatch,
        count: files.length,
      });
    } catch (err: any) {
        ...
        <div className="relative rounded-[22px] bg-[#0c0d16]/95 backdrop-blur-xl p-5 sm:p-8 md:p-10 border border-white/5">
          ...
          <div className="text-center mb-6">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
              {isPro ? (
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gradient-to-r from-amber-500/20 to-purple-500/20 border border-amber-500/40 text-amber-300 text-xs font-semibold shadow-sm">
                  ...
                  <span>Pro Plan Active: Unlimited Conversions & 20-Image Batch Mode</span>
                </div>
              ) : (
            ...
            <p className="text-xs sm:text-sm text-slate-400 mt-1 max-w-lg mx-auto">
              Upload multiple images to merge into one PDF. Free users get <span className="text-purple-300 font-semibold">20 files/day</span>. Premium subscribers can convert up to <span className="text-fuchsia-300 font-semibold">20 images at once</span> with Pro Batch Mode.
            </p>
            ...
            <div className="inline-flex items-center p-1 rounded-xl bg-slate-900 border border-white/10 mt-5 max-w-full">
              ...
              >
                ...
                <span>Pro Batch (Up to {effectiveBatchLimit} Images)</span>
                {!isPro && (
          ...
          {result ? (
            <div className="py-8 px-4 text-center flex flex-col items-center animate-in zoom-in-95 duration-200">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-white mb-1">
                {result.isBatch ? `${result.count} Images Merged into PDF!` : 'Your PDF is Ready!'}
              </h3>
              <p className="text-sm text-slate-400 mb-6">
                {result.isBatch
                  ? `All ${result.count} images were merged into a single PDF document with original quality preserved.`
                  : 'Successfully converted with original resolution and quality preserved.'}
              </p>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs sm:text-sm text-slate-300 px-1">
                  <span className="font-semibold flex items-center gap-2">
                    ...
                    <span>{files.length} Image{files.length > 1 ? 's' : ''} Selected{files.length > 1 ? ' (Will Merge into 1 PDF)' : ''}</span>
                  </span>
                ...
                {files.length < effectiveBatchLimit && (
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    className="w-full p-3 rounded-xl border-2 border-dashed border-white/10 hover:border-purple-500/30 text-slate-400 hover:text-purple-300 text-xs font-medium flex items-center justify-center gap-2 transition"
                  >
                    <UploadCloud size={14} />
                    <span>Add More Images ({files.length}/{effectiveBatchLimit})</span>
                  </button>
                )}
              </div>
                ...
                >
                  {isConverting ? (
                    ...
                    <>
                      ...
                      <span>
                        {files.length > 1
                          ? `Merge ${files.length} Images into PDF`
                          : 'Convert to PDF Now'}
                      </span>
                    </>
                  )}
            ...
            >
              ...
              <h3 className="text-lg sm:text-xl font-bold text-white mb-2 group-hover:text-purple-200 transition">
                Click to browse or drop your images here
              </h3>
              ...
              <p className="text-xs sm:text-sm text-slate-400 max-w-md mx-auto mb-4">
                ...
                <span className="text-purple-300 font-semibold">.JPG</span>,{' '}
                <span className="text-purple-300 font-semibold">.JPEG</span>, and{' '}
                <span className="text-purple-300 font-semibold">.WEBP</span> — upload multiple to merge into one PDF
              </p>
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-purple-600/20 border border-purple-500/40 text-purple-300 text-xs font-semibold group-hover:bg-purple-600/30 transition">
                <ImageIcon size={14} />
                <span>Select Images (Free 20/day)</span>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pt-6 border-t border-white/5 text-slate-400 text-xs text-center sm:text-left">
            ...
            <div className="flex items-center justify-center sm:justify-start gap-2">
              ...
              <span>Pro: Batch 20 Images at Once</span>
            </div>
            <div className="flex items-center justify-center sm:justify-start gap-2">
        ...
        >
          ...
          >
            ...
            <p className="text-xs sm:text-sm text-slate-300 leading-relaxed mb-6">
              Free users can convert <strong className="text-purple-300">up to 20 images per day</strong>.
              <br className="hidden sm:inline" />
              Upgrade to <strong className="text-amber-300">Axom AI Premium</strong> for <strong className="text-white">unlimited daily conversions</strong> and batch convert up to 20 images simultaneously!
            </p>
            ...
            <div className="flex flex-col gap-2.5">
              ...
              >
                Continue with Free Mode (20 files/day)
              </button>
            </div>
          </div>
        </div>,
